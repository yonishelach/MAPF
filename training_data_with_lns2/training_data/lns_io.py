...
def create_lns_input_line(request: (int, int)):
    ...
    """
    return "0\t" + MAP_FILE + "\t" + str(WAREHOUSE_HEIGHT) + "\t" + str(WAREHOUSE_WIDTH) + "\t" + \
           "\t".join([str(x) for x in request]) + "\t" + \
           str(norm([request[0] - request[2], WAREHOUSE_WIDTH - 1], 2)) + "\n"


def create_lns_input_files(requests_batches_without_time):
    """
    Creates lns input file for each batch in requests_without_time_batches.
    """
...
def create_lns_output_file(index: str):
    ...
    command_middle = '.scen -o test.csv -k ' + str(BATCH_SIZE) + ' -t ' + str(MAX_LNS_RUN_TIME) + \
