...
def is_valid_path_steps(path):
    ...
    for i in range(2, len(path), 2):
        ...
        if abs(current_step[0]-next_step[0]) not in [0, 1]:
            print("abs(current_step[0]-next_step[0]) not in [0, 1]")
            return False
        if abs(current_step[1]-next_step[1]) not in [0, 1]:
            print("abs(current_step[1]-next_step[1]) not in [0, 1]")
            return False
        if current_step[0] != next_step[0] and current_step[1] != next_step[1]:
            print("current_step[0] != next_step[0] and current_step[1] != next_step[1]:")
            return False
        if current_step[1] < next_step[1]:
            print("current_step[1] < next_step[1]")
            return False
        if current_step[1] == 0:
            print("current_step[1] == 0:")
            return False
        current_step = next_step
        print("GOT TO NEXT LOOP")
    return True
...
def is_collision_exists(path1, path2):
    ...
    for i in range(0, min(len(path1), len(path2)), 2):
        next_step1 = path1[i:i+2]
        next_step2 = path2[i:i+2]
        if next_step1[1] == 0 or next_step2[1] == 0:
            continue
        if next_step1[0] == next_step2[0] and next_step1[1] == next_step2[1] and next_step1[1] != WIDTH - 1:
            return True
